...
public class MerchantController {
    ...
    @Autowired
    private CurrentMerchantId currentUserProvider;
    ...
    // SECURE: fetches the payout only if it belongs to the currently authenticated merchant.
    @GetMapping("/api/payouts/{payoutId}")
    public PayoutRequest getPayout(@PathVariable Long payoutId) {
        PayoutRequest payout = payoutRepository.findById(payoutId)
                .orElseThrow(() -> new RuntimeException("Payout not found"));
        
        Long currentMerchantId = currentUserProvider.getMerchantId();
        if (!payout.getMerchantId().equals(currentMerchantId)) {
